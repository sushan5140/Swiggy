...
const BLOCK = new Set([
  ...
  "DELIVERED_BUDGET_EXCEEDED","ADDRESS_MISMATCH","NON_GROCERY_ITEM"
]);
...
export function inspect(caseData) {
  ...
  for (const item of items) {
    ...
    if (!p.role || !validMoney(p.pack_g)) add(reasons,"NON_GROCERY_ITEM");
  ...
  for (const swap of proposal.substitutions ?? []) {
    if (!swap.approved) add(reasons,"SUBSTITUTION_NOT_APPROVED");
  }
